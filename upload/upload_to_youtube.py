"""
YouTube Upload Script - VELOCITY CZECH
"""
import os
...
def generate_video_metadata(category, num_phrases, phrases=None):
    title = f"Czech Learning: {num_phrases} Essential {category} Phrases"
    lines = [f"Learn Czech with VELOCITY CZECH!", "", f"Category: {category}", "", f"Master Czech one phrase at a time! Today's {category} lesson:", ""]
    if phrases:
        for i, p in enumerate(phrases[:5], 0):
            lines.append(f"{i+1}. {p['english']}")
            lines.append(f"   {p.get('czech', '')}")
            lines.append(f"   [{p.get('transliteration', '')}]")
            lines.append("")
    lines.extend(["Tip: Repeat each phrase out loud 3 times!", "Like this video if you learned something new!", "Subscribe for daily Czech lessons!", "", f"#LearnCzech #CzechLessons #CzechForBeginners #LanguageLearning", f"#Czech #Education #Tutorial #DailyCzech #{category.replace(' ', '')}", f"#VELOCITYCZECH #CzechPhrases #SpeakCzech"])
    tags = [f"learn czech", f"czech lessons", f"czech for beginners", f"czech phrases", "language learning", f"speak czech", category.lower(), "education", f"daily czech", "velocity czech", f"czech learning"]
    return title, "\n".join(lines), tags
...
def main():
    ...
    try: upload_to_youtube(video_path=video_file, title=f"Learn Czech Daily", description=f"#shorts #learnczech #czech #language #education", tags=[hashtag, 'education', 'language learning'], category_id='22')
