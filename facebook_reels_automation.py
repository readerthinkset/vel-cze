"""
Facebook Reels Automation - Bilingual English/Czech Content Generator
IMPROVED VERSION: Better backgrounds, English categories, no repeats, VELOCITY CZECH branding
...
CATEGORIES_NATIVE = {
    "Greetings": "Pozdravy",
    "Basic Phrases": "Základní Fráze",
    "Common Expressions": "Běžné Výrazy",
    "Travel": "Cestování",
    ...
    "Wisdom": "Moudrost",
    "Happiness": "Štěstí",
    "Self Improvement": "Sebezdokonalování",
    "Gratitude": "Vděčnost",
...
NATIVE_VOICE = "cs-CZ-AntoninNeural"
...
2. Czech translation (in Czech script)
3. Transliteration (Roman script pronunciation)

Return as JSON array:
[{{"english": "...", "czech": "...", "transliteration": "..."}}]

IMPORTANT: Create FRESH, UNIQUE phrases that haven't been used before.
IMPORTANT: Czech text must be clean - no slashes, no multiple versions."""
            ...
            for p in phrases:
                ...
                if "czech" not in p:
                    alt_keys = ["czech_text", "native", "translation", p.get("language", "")]
                    for k in alt_keys:
                        if k in p:
                            p["czech"] = p.pop(k)
                            break
...
def generate_all_audio(phrases: list, output_dir: str):
    ...
    for i, phrase in enumerate(phrases):
        ...
        print(f"    CS: {phrase['czech']}")

        nat_success = asyncio.run(generate_audio_with_retries(phrase["czech"], NATIVE_VOICE, str(native_file)))
        if nat_success:
            print(f"    - Czech: {native_file.name}")
        else:
            print(f"    - Czech: SILENCE FALLBACK (TTS failed)")
        ...
        print(f"    Total: {total_duration:.2f}s (EN: {en_duration:.2f}s + pause: {pause_between}s + CS: {nat_duration:.2f}s)")
...
def find_font(bold=False, size=40):
    ...
    font_file = FONTS_DIR / "NotoSansCzech-Bold.ttf"
    if font_file.exists():
        try:
...
def create_video_from_images_audio(image_files: list, audio_files: list, combined_audio: str, output_file: str):
    ...
    for i, (img_path, audio_info) in enumerate(zip(image_files, audio_files)):
        ...
        print(f"  Image {i+1}/{len(image_files)}: {duration:.2f}s (EN: {audio_info.get('en_duration', 0):.1f}s + CS: {audio_info.get('nat_duration', 0):.1f}s)")
...
def generate_reel(category_english: str = None):
    ...
    for i, phrase in enumerate(phrases, 1):
        print(f"  {i}. {phrase['english']} -> {phrase['czech']}")
    ...
    print("\n[3/4] Generating audio (English + Czech with 500ms pause)...")
    ...
    print(f"  Branding: VELOCITY CZECH")
    print(f"{'='*80}\n")
...
if __name__ == "__main__":
    print("\n" + "="*80)
    print(f"VELOCITY CZECH - FACEBOOK REELS AUTOMATION")
    print("="*80)
    print("\nFEATURES:")
    ...
    print(f"  - VELOCITY CZECH branding at bottom")
